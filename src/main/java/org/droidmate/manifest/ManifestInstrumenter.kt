...
class ManifestInstrumenter(pathToAndroidManifest: Path) {
    ...
    fun changeMinSDKVersion(xmlModifier: XMLModifier) {
        val nav = nav
        if (!nav.toElement(VTDNav.FIRST_CHILD)) {
            return
        }
        do {
            ...
            if (s == "uses-sdk") {
                val ap = AutoPilot(nav)
                ap.selectAttr("minSdkVersion")
                var minSdk: Int = 0
                var i = ap.iterateAttr()
                while (i != -1) {
                    val minVersionString = nav.toString(i + 1)
                    minSdk = minVersionString.toInt()
                    i = ap.iterateAttr()
                }
                ap.selectAttr("targetSdkVersion")
                var targetSdk: Int = 0
                i = ap.iterateAttr()
                while (i != -1) {
                    val targetSdkVersionString = nav.toString(i + 1)
                    targetSdk = targetSdkVersionString.toInt()
                    i = ap.iterateAttr()
                }
                if (minSdk < 22) {
                    ap.selectAttr("minSdkVersion")
                    var j = ap.iterateAttr()
                    xmlModifier.remove()
                    xmlModifier.insertAfterElement(String.format("\n<uses-sdk android:minSdkVersion=\"%d\"" +
                            " android:targetSdkVersion=\"%d\" />", 22, targetSdk))
                }
            }
        } while (nav.toElement(VTDNav.NS))
    }
    ...
    fun changeMinSDKVersionAndWriteOut() {
        val nav = nav
        val modifier = XMLModifier(nav)
        changeMinSDKVersion(modifier)
        modifier.output(filePath)
    }
}
