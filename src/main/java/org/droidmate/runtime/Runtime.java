	...
	public static void addCurrentWidgetPoint(int resourceId, String resourceIdName, String portFilePath, int print)
	{
		synchronized (currentWidgetIds) {
			if (!initialized)
				initialize(portFilePath);
//			log.info("Monitor_API_method_call: " + method);
			currentWidgetIds.put(resourceId,resourceIdName);
			if (print!=0)
			{
				log.log(Level.INFO, "Widget: " + resourceId + ": " + resourceIdName);
			}
		}
	}

	public static void setCurrentActivityPoint(String currentActivityName, String portFilePath, int print )
	{
		if (!initialized)
			initialize(portFilePath);
//			log.info("Monitor_API_method_call: " + method);
		currentActivity.add(currentActivityName) ;
		if (print!=0)
		{
			log.log(Level.INFO, "Activity: " +currentActivityName);
		}
	}
	...
	private final static HashMap<Integer, String> currentWidgetIds = new HashMap<Integer, String>();
	private final static ArrayList<String> currentActivity = new ArrayList<>();
