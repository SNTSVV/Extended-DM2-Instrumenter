...
class MonitorTcpServer extends org.droidmate.runtime.TcpServerBase<String, LinkedList<ArrayList<String>>> {
	...
	private HashMap<Integer, String> currentWidgetIds;

	MonitorTcpServer(LinkedList<ArrayList<String>> currentStatements, ArrayList<String> currentActivity, HashMap<Integer, String> currentWidgetIds) {
		super();

		this.currentStatements = currentStatements;
		this.currentActivity = currentActivity;
		this.currentWidgetIds = currentWidgetIds;
	}
	...
	protected LinkedList<ArrayList<String>> OnServerRequest(String input) {
		...
		synchronized (currentStatements) {
			...
			String srvCmd_get_current_activity = "getCurrentActivity";
			String srvCmd_get_current_widgetIds = "getCurrentWidgetIds";
			...
			if (srvCmd_connCheck.equals(input)) {
				...
				return logsToSend;
			} else if (srvCmd_get_current_activity.equals(input)){
				//log.info("getCurrentActivity" + currentActivity);
				...
				{
					result.add(currentActivity.get(currentActivity.size()-1));
				}

				return new LinkedList<>(Collections.singleton(new ArrayList<String>(result)));
			} else if (srvCmd_get_current_widgetIds.equals(input)){
				log.info("getCurrentWidgetIds" + currentWidgetIds.size());
				LinkedList<ArrayList<String>> logsToSend = new LinkedList<>();
				currentWidgetIds.forEach((key,value) -> {
					logsToSend.add(new ArrayList<>(Arrays.asList(key.toString(), value)));
				});
				currentWidgetIds.clear();
				return logsToSend;
			}
