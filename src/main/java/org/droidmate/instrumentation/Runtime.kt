...
class Runtime private constructor(private val portFile: Path) {
    ...
    private fun getSetCurrentActivityPointMethod(): SootMethod {
        return setCurrentActivityPointMethod.get()
    }
    ...
    fun makeCallToAddCurrentWidgetPoint(resourceId: Int, view: String, printToLogcat: Int): InvokeStmt {
        return Jimple.v().newInvokeStmt(
            Jimple.v().newStaticInvokeExpr(
                this.getAddCurrentWidgetPointMethod().makeRef(),
                IntConstant.v(resourceId),
                StringConstant.v(view),
                StringConstant.v(portFile.toString()),
                IntConstant.v(printToLogcat)
            )
        )
    ...
    fun makeCallToSetCurrentActivity(activityName: String, printToLogcat: Int): InvokeStmt {
        return Jimple.v().newInvokeStmt(
            Jimple.v().newStaticInvokeExpr(
                this.getSetCurrentActivityPointMethod().makeRef(),
                StringConstant.v(activityName),
                StringConstant.v(portFile.toString()),
                IntConstant.v(printToLogcat)
            )
        )
