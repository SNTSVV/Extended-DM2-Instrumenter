...
public class Hierarchy {
  ...
  //Fragment
  public boolean isFragmentClass(final SootClass c) {
    return isSubclassOf(c, Scene.v().getSootClass("androidx.fragment.app.Fragment")) || isSubclassOf(c, Scene.v().getSootClass("android.support.v4.app.Fragment"));
  }
    ...
    if (!receiverClass.isConcrete()) {
       if (sanityCheck)
       {
         throw new RuntimeException(
          ...
          log.warn("Hierarchy.virtualDispatch called with non-concrete receiver class: " + receiverClass.getName());
    }
    // look up the method
  ...
  public SootMethod virtualDispatch(String staticTargetSubsig,
    ...
    if (!receiverClass.isConcrete()) {
      if (sanityCheck) {
        ...
        log.warn("Hierarchy.virtualDispatch called with non-concrete receiver class: " + receiverClass
                + ", subsig: " + staticTargetSubsig);
      }
    }
    // look up the method
  ...
  private Hierarchy() {
    ...
    log.trace("TIMECOST", "HIER parsing finished at " + System.currentTimeMillis());
  } // Hierarchy()
  ...
  void recordFrameworkInvokedCallbacks() {
    ...
    for (SootClass c : appClasses) {
      ...
      if (!interesting) {
        //Logger.verb(this.getClass().getSimpleName(), "Not interesting: "+c.getName());
        continue;
      }
      ...
      if (instantiated) {
        //Logger.verb(this.getClass().getSimpleName(),"Is instantiated:  "+c.getName());
        continue;
      }
      ...
      if (!applicationActivityClasses.contains(c) && !applicationFragmentClasses.contains(c)) {
        log.info("Not application activity and fragment class: "+c.getName());
        continue;
      }
      ...
      for (String sub : allSubsig) {
        ...
        for (SootClass t = c; t.isApplicationClass(); t = t.getSuperclass()) {
          if (t.declaresMethod(sub)) {
            ...
            if (!m.isConcrete()) {
              log.warn( "Callback method :" + m.getName() + " in Class " + t.getName()
                      + " is not concrete");
              continue;
            }
            callbacks.add(m);
  ...
  void fragmentClasses() {
    SootClass frag1 = Scene.v().getSootClass("androidx.fragment.app.Fragment");
    SootClass frag2 = Scene.v().getSootClass("android.support.v4.app.Fragment");
    if (frag1 == null && frag2 == null){
      throw new Error("[HIER] Did not find Fragment");
    }
    for (SootClass c: appClasses) {
      if (getSupertypes(c).contains(frag1) || getSupertypes(c).contains(frag2)){
        applicationFragmentClasses.add(c);
      }
    }
    for (SootClass c: Scene.v().getLibraryClasses()){
      if (getSupertypes(c).contains(frag1) || getSupertypes(c).contains(frag2)){
        libFragmentClasses.add(c);
      }
    }
    log.info(
            "[HIER] Fragments: " + applicationFragmentClasses.size()
                    + ", lib fragments: " + libFragmentClasses.size());
  }
...
//    }
  ...
  void buildApplicationCodeMap() {
    log.trace("TIMECOST", "buildApplicationCodeMap parsing starting at " + System
            .currentTimeMillis());
    ...
    log.trace("TIMECOST", "buildApplicationCodeMap parsing finished at " + System
            .currentTimeMillis());
  }
  ...
  //Fragment classes
  public Set<SootClass> applicationFragmentClasses = Sets.newHashSet();
  public Set<SootClass> libFragmentClasses = Sets.newHashSet();
}
